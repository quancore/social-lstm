...
def main():
    ...
    create_directories(result_directory, dataloader.get_all_directory_namelist())
    ...
    for iteration in range(sample_args.iteration):
        ...
        for batch in range(dataloader.num_batches):
            ...
            if dataset_pointer_ins is not dataloader.dataset_pointer:
                #print("iteration write")
                #print("first point :%s"%submission[0])
        ...
        iteration_submission.append(submission)
        iteration_result.append(results)
        #print("final iteration write")

        submission_store.append(iteration_submission)
        #print("first point :%s"%submission_store[iteration][0][0:1])
    ...
    #dataloader.write_to_plot_file(result_store[smallest_err_iter_num], os.path.join(f_prefix, plot_directory))
    dataloader.write_to_plot_file(result_store[smallest_err_iter_num], os.path.join(plot_directory, plot_test_file_directory))


def sample(x_seq, Pedlist, args, net, true_x_seq, true_Pedlist, true_grid, saved_args, dimensions, dataloader, look_up, num_pedlist, is_gru, grid = None):
    '''
    The sample function
    params:
    ...
    #print("received pedlist: %s"%true_Pedlist)
    with torch.no_grad():
        ...
        ret_x_seq = Variable(torch.zeros(args.obs_length+args.pred_length, numx_seq, 2), volatile=True)
        ...
        for tstep in range(args.obs_length-1, args.pred_length + args.obs_length-1):
            #print("**********************************")
            #print(ret_x_seq[tstep].view(1, numx_seq, 2))
            # Do a forward prop
            if grid is None: #vanilla lstm
            ...
            # loss_pred = Gaussian2DLikelihoodInference(outputs, true_x_seq[tstep+1].view(1, numx_seq, 2), Pedlist[args.obs_length-1], [true_Pedlist[tstep+1]])
            ...
            list_of_x_seq = Variable(torch.LongTensor(converted_pedlist), volatile=True)

            if args.use_cuda:
                list_of_x_seq = list_of_x_seq.cuda()
            #Get their predicted positions
            #current_x_seq = torch.index_select(ret_x_seq[tstep+1], 0, np.array(target_id))
            ...
            if grid is not None: #no vanilla lstm
                ...
                prev_grid = getGridMaskInference(current_x_seq.data.cpu().numpy(), dimensions, saved_args.neighborhood_size, saved_args.grid_size)
...
def submission_preprocess(dataloader, ret_x_seq, pred_length, obs_length, target_id):
    ...
    ret_x_seq_c[:,[0,1]] = ret_x_seq_c[:,[1,0]]
    #print("target_id: ", target_id)
    #print("values ", first_value_dict[target_id])
    #print(observed_data)
    #print(ret_x_seq_c)
    #ret_x_seq_c[:, 0] = [point[0]+first_value_dict[target_id][1] for point in ret_x_seq_c]
    #ret_x_seq_c[:, 1] = [point[1]+first_value_dict[target_id][0] for point in ret_x_seq_c]
    ...
    #print("******************************************")
    #print(observed_data)
    #print(frame_number_predicted.shape)
    repeated_id = np.repeat(target_id, pred_length)
